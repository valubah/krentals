            ...
            flexibleSpace: FlexibleSpaceBar(
              ...
              background: Stack(
                ...
                children: [
                  ...
                  // Page Indicator — deferred one frame so controller is attached
                  Positioned(
                    bottom: 44,
                    right: 20,
                    child: _SafePageIndicator(
                      controller: _pageController,
...
// ── Safe Page Indicator ────────────────────────────────────────────────────
// Defers rendering by one frame so PageController is attached to PageView first
class _SafePageIndicator extends StatefulWidget {
  final PageController controller;
  final int count;
  const _SafePageIndicator({required this.controller, required this.count});

  @override
  State<_SafePageIndicator> createState() => _SafePageIndicatorState();
}

class _SafePageIndicatorState extends State<_SafePageIndicator> {
  bool _ready = false;
...
// lib/presentation/screens/car_detail_screen.dart
//
// DIAGNOSTIC VERSION — hardcoded data, zero external dependencies,
// proves whether the issue is data/blocs or the widget tree itself.
                  ...
                  PageView.builder(
                    ...
                    itemBuilder: (context, i) {
                      return Stack(
                        fit: StackFit.expand,
                        children: [
                          // NO Hero here — the Hero only lives in the source
                          // card. Having Hero on both source and destination
                          // caused the Hero overlay to blank the entire screen.
                          AppImage(
                            path: car.imageUrls[i],
                            fit: BoxFit.cover,
                          ),
        ...
        children: [
          ...
          const Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Hosted by K Rentals',
                    style: TextStyle(fontWeight: FontWeight.w900, fontSize: 17)),
                SizedBox(height: 2),
                Text('All-Star Host • 4.98 Rating',
                    style: TextStyle(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          const Icon(Icons.check_rounded, color: AppTheme.success, size: 16),
          const SizedBox(width: 8),
          Text(feature, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
        ],
      ),
    );
  }
